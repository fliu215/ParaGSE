...
from quantize_gvq import ResidualVectorQuantize
...
class Encoder(torch.nn.Module):
    def __init__(self, h):
        ...
        self.input_channels=h.hop_size
        self.h=h
        self.dim=256
        self.num_layers=8
        ...
        self.AMP_Encoder_downsample_output_conv = weight_norm(Conv1d(h.AMP_Encoder_channel, h.AMP_Encoder_channel, h.AMP_Encoder_output_downconv_kernel_size, h.ratio, 
                                                  padding=get_padding(h.AMP_Encoder_output_downconv_kernel_size, 1)))

        self.latent_output_conv = weight_norm(Conv1d(h.AMP_Encoder_channel, h.latent_dim, h.latent_output_conv_kernel_size, 1, 
        ...
        self.quantizer = ResidualVectorQuantize(
            ...
            codebook_dim=h.latent_dim//4,
            n_codebooks=h.num_quantize,
    ...
    def forward(self, MDCT):
        ...
        MDCT_encode = self.embed_logamp(MDCT)
        MDCT_encode = self.norm_logamp(MDCT_encode.transpose(1, 2))
        MDCT_encode = MDCT_encode.transpose(1, 2)
        for conv_block in self.convnext_logamp:
            MDCT_encode = conv_block(MDCT_encode, cond_embedding_id=None)
        MDCT_encode = self.final_layer_norm_logamp(MDCT_encode.transpose(1, 2))
        MDCT_encode = self.out_logamp(MDCT_encode).transpose(1, 2)
        MDCT_encode = self.AMP_Encoder_downsample_output_conv(MDCT_encode)

        encode = MDCT_encode

        latent = self.latent_output_conv(encode)
        z,codes,latents,commitment_loss,codebook_loss = self.quantizer(latent)
...
class Decoder(torch.nn.Module):
    def __init__(self, h):
        ...
        self.latent_input_conv = weight_norm(Conv1d(h.latent_dim, h.AMP_Decoder_channel, h.latent_input_conv_kernel_size, 1, 
                                                 padding=get_padding(h.latent_input_conv_kernel_size, 1)))

        self.AMP_Decoder_upsample_input_conv = weight_norm(ConvTranspose1d(h.AMP_Decoder_channel, h.AMP_Decoder_channel,
        ...
        self.PHA_Decoder_output_R_conv = weight_norm(Conv1d(h.PHA_Decoder_channel, h.hop_size, h.PHA_Decoder_output_R_conv_kernel_size, 1, 
                                                    padding=get_padding(h.PHA_Decoder_output_R_conv_kernel_size, 1)))
        self.PHA_Decoder_output_I_conv = weight_norm(Conv1d(h.PHA_Decoder_channel, h.hop_size, h.PHA_Decoder_output_I_conv_kernel_size, 1, 
    ...
    def forward(self, latent):
        ...
        # R = self.PHA_Decoder_output_R_conv(logamp)
        # I = self.PHA_Decoder_output_I_conv(logamp)
        MDCT_coff=self.PHA_Decoder_output_R_conv(logamp)
        # MDCT_coff=torch.exp(R)*torch.cos(I)

        MDCT_coff=MDCT_coff.permute(0,2,1)

        audio=IMDCT_operation(MDCT_coff)

        return audio.unsqueeze(1),MDCT_coff.permute(0,2,1)
...
class DiscriminatorR(nn.Module):
    ...
    def forward(
        ...
        fmap = []
        # print(x.shape)torch.Size([16, 1, 7960])
        x=x.squeeze(1)
        # print("squeeze",x.shape)torch.Size([16, 1, 7960])
        x = self.spectrogram(x)
        # print(x.shape)torch.Size([16, 257, 399])torch.Size([16, 513, 200])torch.Size([16, 1025, 100])
        x = x.unsqueeze(1)
        for l in self.convs:
            x = l(x)
    ...
    def spectrogram(self, x: torch.Tensor) -> torch.Tensor:
        hop_length = self.resolution
        MDCT_operation=MDCT(hop_length).to(device)
        
        magnitude_spectrogram = MDCT_operation(
            x,
        )

        magnitude_spectrogram=magnitude_spectrogram.permute(0,2,1)
    ...
    #     magnitude_spectrogram = torch.abs(torch.sqrt(torch.pow(rea,2)+torch.pow(imag,2))) + 1e-5
